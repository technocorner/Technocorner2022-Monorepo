...
export default function Admin() {
  ...
  useEffect(() => {
    (async () => {
      const res = await getData(`/dashboard/webinar/`);
      if (res.success) {
        setData(res.body);
        setLoading(false);
      } else {
  ...
  async function verifyHandler(id: string, verifikasi: boolean) {
    ...
    const res = await putData(`/dashboard/webinar/verifikasi`, {
      id,
    ...
    if (res.success) {
      const index = data.findIndex(
        (d: { idPendaftaran: string }) => id === d.idPendaftaran
      );
      setData((prevData) => {
        const data = [...prevData];
  ...
  return loading ? (
    ...
    <>
      <h1 className="font-[GothamBold] text-2xl">Webinar</h1>
                    ...
                    <tr key={d.idPendaftaran}>
                      ...
                      >
                        <a
                          ...
                          href={`${router.asPath}/peserta/${d.idPendaftaran}`}
                          target="_blank"
                      ...
                      >
                        <div className="flex justify-center flex-wrap gap-1">
                          ...
                          <button
                            ...
                            onClick={() =>
                              verifyHandler(d.idPendaftaran, d.verifikasi)
                            }
                          >
                            {d.verifikasi
                              ? "Batalkan verifikasi"
