...
func InitFsDb() *firestore.Client {
...
func CloseFsDb(client *firestore.Client) {
	client.Close()
}
...
func GetTeamData(client *firestore.Client, id string) (model.DbData, error) {
	...
	if !data.Exists() {
		err = errors.New("Team not exist")
	}

	return model.DbData{ID: data.Ref.ID, Data: data.Data()}, err
}

func GetUserData(client *firestore.Client, id string) (model.DbData, error) {
	ctx := context.Background()
	data, err := client.Collection("pengguna").Doc(id).Get(ctx)
	if !data.Exists() {
