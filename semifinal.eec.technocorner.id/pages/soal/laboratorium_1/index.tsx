...
export default function soal({
  ...
  const [questionLink, _] = useState(
    `${apiServer[0]}/question?lab=${LabNumber.Lab1}&time=${Date.now()}`
  );
  ...
  useEffect(() => {
    if (router.isReady) {
      const interval = setInterval(() => {
        (async () => {
          const res = await getData(`/lab/${LabNumber.Lab1}`);

          if (!res.json) {
            toastError(`Akses ${labs[LabNumber.Lab1 - 1].title} telah ditutup`);
            router.replace("/");
          }
        })();
      }, 1000);
    ...
    <>
      ...
      <div>
        <h1 className="font-bold text-2xl">
          Lab 1 - Laboratorium Digital Mikroprosesor
        </h1>
      </div>
