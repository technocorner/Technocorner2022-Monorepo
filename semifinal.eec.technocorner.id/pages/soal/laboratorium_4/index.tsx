...
export default function soal({
  ...
  const [questionLink, _] = useState(
    `${apiServer[0]}/question?lab=${LabNumber.Lab4}&time=${Date.now()}`
  );
  ...
  useEffect(() => {
    if (router.isReady) {
      const interval = setInterval(() => {
        (async () => {
          const res = await getData(`/lab/${LabNumber.Lab4}`);

          if (!res.json) {
            toastError(`Akses ${labs[LabNumber.Lab4 - 1].title} telah ditutup`);
            router.replace("/");
          }
        })();
      }, 1000);
  ...
  return (
    <>
      <HtmlHead
        title="Lab 4"
        description="Lab 4 semifinal kompetisi EEC yang diselenggarakan oleh Technocorner. Semifinal ini akan diadakan pada tanggal 19-20 April 2022."
      />
      <div>
        <h1 className="font-bold text-2xl">
          Lab 4 - Laboratorium Elektronika Dasar
        </h1>
      </div>
